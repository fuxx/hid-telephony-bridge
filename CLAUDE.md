...
python3 mv7-hid-bridge.py --vid 1234 --pid 5678 --verbose
...
- **`MV7Bridge`** — opens the hidraw device, sends the Off-Hook handshake to activate telephony mode, polls for HID input reports, toggles PipeWire source mute via `pactl`, and syncs the mute LED. The main loop uses `select.poll()` on the hidraw fd with a 500ms timeout.
...
Supporting files:
- `99-shure-mv7-hid.rules` — udev rule (hidraw permissions for `input` group)
- `mv7-hid-bridge.service` — systemd user service (bound to `pipewire-pulse.service`)
- `install.sh` — installs script, udev rule, and service to system paths
- `PKGBUILD` + `mv7-hid-bridge.install` — AUR packaging
